...
| ⚠️ⓘ❗ **NOTE** |
|---------------|
| Obtain an Infura API key by creating an account at [infura.io](https://infura.io) $\color{red}{(***MetaMask\ wallet\ login\ supported***\)}$. Avoid using MetaMask’s default Infura key due to rate limits, as it is shared and heavily restricted. Using mainnet incurs $\color{red}{***higher\ Infura\ API\ costs***\}$ and interacts with <span style="color:red">***real***</span> Ethereum transactions. Ensure your Infura API key supports mainnet and testnet access and use <span style="color:red">***cautiously***</span> in production environments. |
